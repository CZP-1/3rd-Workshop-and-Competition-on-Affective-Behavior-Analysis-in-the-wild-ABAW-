...
class Aff2_Dataset_static_multitask_mixup(Dataset):
    def __init__(self, transform, df=None, mode="val"):
        super(Aff2_Dataset_static_multitask_mixup, self).__init__()
        self.df = df
        self.transform = transform
        self.mode = mode
        self.list_labels_ex = np.array(self.df['labels_ex'].values)
    ...
    def __getitem__(self, index):
        ...
        label = torch.zeros(8)
        label[self.list_labels_ex[index]] = 1
        if self.transform:
            image = self.transform(image)
        if self.mode == 'train' and index%5 == 0:
            mixup_index = random.randint(0,len(self.df)-1)
            mix_label = torch.zeros(8)
            label[self.list_labels_ex[mixup_index]] = 1
            mixup_image = cv2.imread(self.list_image_id[mixup_index])[..., ::-1]
            if self.transform:
