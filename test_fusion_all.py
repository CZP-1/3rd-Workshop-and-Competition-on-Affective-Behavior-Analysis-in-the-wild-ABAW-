...
def main():
    ...
    model16 = facenet(num_classes_ex=8)
    checkpoint16 = torch.load('weights_kfold/InceptionResnetV1_all.pth')
    model16.load_state_dict(checkpoint16)

    model17 = r50_vgg(num_classes=8)
    checkpoint17 = torch.load('weights_kfold/r50_vgg_all.pth')
    model17.load_state_dict(checkpoint17)

    # model3 = efficientnet_b0(num_classes=8,pretrained=True)
    model18 = my_efficientnetb0(num_classes_ex=8)
    checkpoint18 = torch.load('weights_kfold/efficientnet_b0_all.pth')
    model18.load_state_dict(checkpoint18)
    ...
    for ind, name_video in tqdm(enumerate(df_video['folder_dir']), total=len(df_video)):
        ...
        with torch.no_grad():
            ...
            for batch_idx, samples in tqdm(enumerate(test_loader_ex), total=len(test_loader_ex),
                ...
                pred_cat16 = model16(images)
                pred_cat17 = model17(images)
                pred_cat18 = model18(images)

                pred_cat16 = F.softmax(pred_cat16)
                pred_cat17 = F.softmax(pred_cat17)
                pred_cat18 = F.softmax(pred_cat18)

                pred_cat_6 = pred_cat_1+pred_cat_2+pred_cat_3+pred_cat_4+pred_cat_5
                pred_cat_7 = pred_cat16+pred_cat17+pred_cat18

                pred_cat = 0.4*pred_cat_7 + 0.6*pred_cat_6
